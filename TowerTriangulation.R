####    code for triangulating the radio tower data for summer 2023

# will need to add manually located occurrences

# first need to remove any duplicate values from the radio tower data
library(tidyverse)

tags <- read.csv("tagsProcessed.csv") #only 166 obs, could be a problem
...
tagsTime <- separateTime(tags$GPStime)
tags[,"uniqueID"] <- paste0(tags$AnimalID, "-",tags$GPSdate,"-",
                             tagsTime$hours)
...
locate <-function(x) {
  ...
  for (group in unique(intersections$uniqueID)) {
    tryCatch({
      ...
      valid=TRUE
      if (location[1]<min(intersections$X[intersections$uniqueID==group]-buffer, na.rm=T)) valid=FALSE
      if (location[1]>max(intersections$X[intersections$uniqueID==group]+buffer, na.rm=T)) valid=FALSE
      if (location[2]<min(intersections$Y[intersections$uniqueID==group]-buffer, na.rm=T)) valid=FALSE
      if (location[2]>max(intersections$Y[intersections$uniqueID==group]+buffer, na.rm=T)) valid=FALSE
...
newdata<-left_join(tags, test, by="uniqueID")
